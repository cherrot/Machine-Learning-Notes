...
\begin{figure}[!tbh]
\begin{centering}
\includegraphics[width=0.7\paperwidth]{7\lyxdot 2\lyxdot Intuition2}
\par\end{centering}
...
\caption{Gradient Descent with Regularization}
...
上式中的矩阵是 $\left(n+1\right)\times\left(n+1\right)$.其实等号后的部分正是令Cost
function 求导为0的结果：$\frac{\delta}{\delta\theta_{j}}J\left(\theta\right)\overset{set}{=}0$。具体为什么老师不解释:D

我们知道，当 $m\leq n$ 时，$X^{T}X$ 是不可逆的，然而，当使用 Regularization方法后，得到的矩阵一定是可逆的。


\subsection{Regularized logistic regression}

\includegraphics[width=0.8\paperwidth]{\string"7.4.Regularized logistic regression\string".png}
...
\includegraphics[width=0.8\paperwidth]{7\lyxdot 4\lyxdot CostFuntion}

这里仍有一处符号错误，$\frac{\lambda}{m}\theta_{j}$ 前应该是+。

同前面一样，中括号中的部分仍是 $J\left(\theta\right)$ 的偏导。


\subsubsection{Advanced optimization}

\includegraphics[scale=0.7]{\string"7.4.Advanced optimization\string".png}
